...
int main() {
    ...
    printf("Digite o nome da cidade (ex: PortoAlegre): ");
    scanf(" %s", nomeCidade1);
    ...
    mediaPIB1 = (pib1 * 1000000000.0f) / populacao1;
    mediaPIB2 = (pib2 * 1000000000.0f) / populacao2;

    // Cálculo do Super Poder para cada carta
    superPoder1 = (float)populacao1 + area1 + pib1 + pontosTuristicos1 + mediaPIB1 + (1.0f / densPop1);
    superPoder2 = (float)populacao2 + area2 + pib2 + pontosTuristicos2 + mediaPIB2 + (1.0f / densPop2);
    ...
    // Utilização do operador ternário com comparação (vence maior entre A e B)) para todos os atributos, 
    // para a comparação da Densidade Ppulacional, cuja lógica vencedora é invertida (vence menor entre A e B) 
